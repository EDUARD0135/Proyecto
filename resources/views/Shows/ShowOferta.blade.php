...
@section('titulo', 'Descripcion de la Oferta')
...
@section('todo')
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center">
                                <h1 class="h2" style="text-align: justify; width: 590px; color:black">
                                    <b>{{ $oferta->nombre }}</b>
                                </h1>
                                <button type="button" style="background-color: #a4ff87;" class="btn btn"
                                    onclick="window.location.href='{{ route('vertienda', ['id' => $oferta->usuario_id]) }}'">
                            ...
                            @if (session('usuario'))
                                <div class="mt-2" style="display: flex;">
                                    <form method="POST" action="{{ route('oferta.like', $oferta->id) }}">
                                        @csrf
                                        <button type="submit" class="btn btn-success"><i class="fa-solid fa-thumbs-up"></i> Me Gusta
                                            ({{ $likes }})</button>
                                    </form>

                                    <form method="POST" action="{{ route('oferta.dislike', $oferta->id) }}" class="mx-2">
                                        @csrf
                            ...
                            <h6><b>Vendedor:</b></h6>
                            @if ($oferta->usuario)
                                <p>{{ $oferta->usuario->nombre ?? 'Nombre no disponible' }}
                                    {{ $oferta->usuario->apellido ?? 'Apellido no disponible' }}
                                </p>
                            @else
    ...
    <!-- INICIO OFERTAS PARECIDAS -->
    <section class="custom-bg py-4">
        <div class="container">
            <div class="row text-left pb-3">
                <h1 class="display-5 text-success"><b>Ofertas Parecidas</b></h1>
            </div>

            <div id="carousel-related-product">
                <div id="content">
                    <div class="row">
                        @forelse($ofertas as $ofertaparedida)
                            <div class="col-md-4 cart">
                                <div class="card mb-4 product-wap rounded-0">
                                    <div class="card rounded-0">
                                        <img style="height: 240px; object-fit: cover;"
                                            src="{{ asset($ofertaparedida->Imagen) }}" alt="Related Product Image">
                                    </div>
                                    <div class="card-body d-flex flex-column align-items-center">
                                        <div class="d-inline-block m-2">

                                            <form
                                                action="{{ route('AgregarFavoritoOferta', ['oferta' => $ofertaparedida->id]) }}"
                                            ...
                                            <a href="{{ route('ShowOferta', ['id' => $ofertaparedida->id]) }}"
                                        ...
                                        <a href="{{ route('ShowOferta', ['id' => $ofertaparedida->id]) }}"
                                            ...
                                            title="{{ $ofertaparedida->nombre }}"><b>{{ $ofertaparedida->nombre }}</b></a>
                                        <span class="h5 text-decoration-none"><b>Lps
                                                {{ $ofertaparedida->precio }}</b></span>
                                        <a class="h5 text-decoration-none text-left">Categoría: <span
                                                style="color: red;"><b>{{ $ofertaparedida->categoria }}</b></span></a>
                                    </div>
                                </div>
                            </div>
                        @empty
                            <div class="container text-center">
                                <p><b>¡Vaya! Parece que no hay mas Ofertas en este Momento.</b></p>
        ...
        <div class="container">
            @if (session('usuario'))
                <div>
                    ...
                    <form action="{{ route('agregar_comentario_oferta', $oferta->id) }}" method="post">
                        @csrf
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right">
                                        @if (session('usuario') && session('usuario')->id == $comentario->usuario->id)
                                            ...
                                            <form method="post"
                                                action="{{ route('EliminarComentarioOferta', [$comentario->id]) }}">
                                                @method('DELETE')
                                                @csrf
